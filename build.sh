	...
	if [ "${BUILD_NO}" == "1" ]; then
		...
		if [ "$1" == "0" ]; then
			if [ "$2" == "0" ]; then
                if [ "${CUSTOM_DTB}" == "1" ]; then
					if [ "${12}" == "2" ]; then
						cp -f out/arch/$ARCH/boot/dtb.img AnyKernel3/
					else
...
######### DTB_GENERATOR 0 0 208000 1690000 1586000 208000 2080000 2080000 1300000 1200000 "1200000 1100000 1001000 845000 676000 545000" 2
        ...
        DTB_GENERATOR 1 0 208000 1690000 1586000 208000 2080000 2080000 1300000 1200000 "1200000 1100000 1001000 845000 676000 545000" 2
	...
	if [ -e "arch/$ARCH/boot/Image" ]; then
		{
			...
			if [ -e "arch/$ARCH/boot/dtb.img" ]; then
				if [ "${CODENAME}" == "A3050" ]; then
					...
					cp -f arch/$ARCH/boot/dtb.img AnyKernel3/dtb.img
				fi
			fi
